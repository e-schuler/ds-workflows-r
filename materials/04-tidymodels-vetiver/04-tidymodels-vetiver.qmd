...
Now you can close the `.Renviron` file and restart your R session (Session–\>Restart R) so that the API key variable get's added to our environment. To confirm it worked, try running the following code:
...
v_model <- vetiver_model(ferry_fit_lr, "e-schuler/ferry_model_eschuler")
...
vetiver_deploy_rsconnect(board = board, name = "e-schuler/ferry_model_eschuler")
